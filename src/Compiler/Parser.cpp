...
bool ParseExpression(VectorView<Token> tokens, ParsingContext& ctx, Expression& outExpr, int& tokensConsumed, ExpressionParsingPrecedence ep)
{
	...
	if (tokens[0].type == TokenType::Symbol)
	{
	    ...
	    if (tokens[0].value == "!")
	    {
		if (outExpr.type == AtomicType::Boolean)
		{
		    outExpr = UnaryExpression{ {AtomicType::Boolean, tokens}, UnaryExpressionType::Not };
		}
		else
		{
		    ctx.errors.push_back({ "Argument of !(x) must be a bool.", tokens[0].line, tokens[0].column });
		    outExpr = UnaryExpression{ {AtomicType::Error, tokens}, UnaryExpressionType::Not };
		}
		return true;
	    }
	    else if (tokens[0].value == "-")
	    {
		if (outExpr.type == AtomicType::Integer)
		{
		    outExpr = UnaryExpression{ {AtomicType::Integer, tokens}, UnaryExpressionType::Minus };
		}
		else if (outExpr.type == AtomicType::Double)
		{
		    outExpr = UnaryExpression{ {AtomicType::Double, tokens}, UnaryExpressionType::Minus };
		}
		else
		{
		    ctx.errors.push_back({ "Argument of -(x) must be a number.", tokens[0].line, tokens[0].column });
		    outExpr = UnaryExpression{ {AtomicType::Error, tokens}, UnaryExpressionType::Minus };
		}
		return true;
	    }
	    else if (tokens[0].value == "+")
		...
		if (outExpr.type == AtomicType::Integer)
		{
		    outExpr = UnaryExpression{ {AtomicType::Integer, tokens}, UnaryExpressionType::Plus };
		}
		else if (outExpr.type == AtomicType::Double)
		{
		    outExpr = UnaryExpression{ {AtomicType::Double, tokens}, UnaryExpressionType::Plus };
		}
		else
		{
		    ctx.errors.push_back({ "Argument of +(x) must be a number.", tokens[0].line, tokens[0].column });
		    outExpr = UnaryExpression{ {AtomicType::Error, tokens}, UnaryExpressionType::Plus };
		}
		return true;
	    }
	    else 
